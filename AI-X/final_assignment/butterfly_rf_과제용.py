...
OUTPUT_DIR = './output'
CSV_DIR = os.path.expanduser('~/Study/CourseWork/Coursework/AI-X/final_assignment/결측행제거')
CSV_PATTERN = '*.csv'
...
if __name__ == "__main__":
    ...
    # --- butterfly_rf.py 파일 맨 아래 수정 예시 ---
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)  # 예측값 먼저 계산
    
    # 1. 기존 정확도 출력
    acc = accuracy_score(y_test, y_pred)
    print(f"\n전체 정확도: {acc:.1%}")

    # [추가] 2. 혼돈 행렬의 결과 비교를 위한 상세 리포트 출력
    print("\n" + "="*50)
    print("      곤충별 상세 평가지표 (정밀도/재현율/F1-score)")
    print("="*50)
    ...
    joblib.dump(model, os.path.join('/Users/hyun/Study/CourseWork/Coursework/AI-X/final_assignment', 'random_forest_컬럼제거.pkl'))
    print (f"모델이 'random_forest_컬럼제거.pkl'에 저장되었습니다.")
