# sobel filter를 적용하여 vertical edge detection을 수행하는 과정을 단계별로 시각화하는 애니메이션입니다.
# horizontal edge detection을 원한다면 kernel을 아래와 같이 변경하면 됩니다.
# kernel = np.array([           
#     [-1, -2, -1],
#     [ 0,  0,  0],
#     [ 1,  2,  1]
...
# simplified kernel로 edge 검출을 직감적으로 (noise 제거 없이)
# sobel filter로 noise 제거된 edge 검출을 시각적으로 비교 (이미지에 노이즈 추가하여)
...
image = np.array([
    [50, 50, 50, 50, 50, 50, 200, 200, 200],
    [50, 50, 50, 50, 50, 50, 200, 200, 200],
    [50, 50, 50, 50, 50, 50, 200, 200, 200],
    [200, 200, 200, 200, 200, 200, 200,200, 200],
    [200, 200, 200, 200, 200, 200, 100, 200, 200],      # [200, 200, 200, 200, 200, 200, 100, 200, 200],
    [200, 200, 200, 200, 200, 200, 200, 200, 200],
    [200, 200, 200, 50, 50, 50, 50, 50, 50],
    [200, 200, 200, 50, 50, 50, 50, 50, 50],
    [200, 200, 200, 50, 50, 50, 50, 50, 50]
])
...
# Reflect Padding 적용 (9x9 -> 11x11)
# for i in range(rows + 2*pad_h):
#     for j in range(cols + 2*pad_w):
#         r = i - pad_h
#         if r < 0: r = -r
#         elif r >= rows: r = 2 * rows - r - 2
        
#         c = j - pad_w
#         if c < 0: c = -c
#         elif c >= cols: c = 2 * cols - c - 2
...
def update(frame):
    ...
    # 데이터 업데이트
    raw_result[r, c] = abs(conv_val)
    threshold_result[r, c] = 255 if (abs(conv_val) > threshold) else 0
    ...
    ax2.set_title(f"2. Raw Sobel (Abs)\nValue: {int(conv_val)}")
